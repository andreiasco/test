...
function adaugaEditorIntrebareQuiz(question = null) {
    ...
    if (type === "multiple_choice") {
        specific = `<div class="quiz-answer-editors">${[0,1,2,3].map((a) => `<label class="quiz-answer-editor-row"><input type="radio" name="quiz-correct-${index}" value="${a}" ${correctIndex===a?"checked":""}><span>${String.fromCharCode(65+a)}</span><input class="quiz-q-answer" type="text" maxlength="220" value="${escapeAdminQuizHtml(answers[a]||"")}" placeholder="Varianta ${String.fromCharCode(65+a)}"></label>`).join("")}</div>`;
        ...
        specific = `<label>Cuvânt / expresie corectă<input class="quiz-q-target" type="text" maxlength="80" required value="${escapeAdminQuizHtml(target)}" placeholder="Ex.: SUBSTANTIV"></label><small class="quiz-admin-hint">Diacriticele sunt acceptate. Spațiile și cratimele se afișează automat.</small>`;
    } else {
        specific = `<label>Elementele în ordinea corectă<textarea class="quiz-q-order" rows="3" maxlength="500" required placeholder="Ex.: introducere, cuprins, încheiere">${escapeAdminQuizHtml(orderItems.join(", "))}</textarea></label><small class="quiz-admin-hint">Separă elementele prin virgulă sau scrie fiecare element pe un rând nou.</small>`;
    }
    ...
    card.innerHTML = `<div class="quiz-question-editor-title"><strong>Scena <span class="quiz-editor-number"></span> · întâlnire cu monstrul</strong><button type="button" class="quiz-remove-question" aria-label="Șterge scena">✕</button></div>
        <div class="quiz-admin-scene-grid"><label>Monstru<select class="quiz-q-monster">${adminMonsterOptions(question?.monster || "goblin")}</select></label><div class="quiz-mode-badge">${QUIZ_MODE_META[mode].icon} ${QUIZ_MODE_META[mode].label}</div></div>
        <label>${type === "hangman" ? "Indiciu / cerință" : type === "ordering" ? "Cerință" : type === "true_false" ? "Afirmație" : "Întrebare"}<textarea class="quiz-q-text" rows="2" maxlength="400" required>${escapeAdminQuizHtml(question?.text || "")}</textarea></label>
        ${specific}
        <label>Explicație după provocare <span class="quiz-optional">(opțional)</span><textarea class="quiz-q-explanation" rows="2" maxlength="400">${escapeAdminQuizHtml(question?.explanation || "")}</textarea></label>
        <div class="quiz-admin-dialog-grid"><label>Replica la succes<input class="quiz-q-correct-message" type="text" maxlength="180" value="${escapeAdminQuizHtml(question?.correctMessage || "Corect! Poți trece mai departe.")}"></label><label>Replica la greșeală<input class="quiz-q-wrong-message" type="text" maxlength="180" value="${escapeAdminQuizHtml(question?.wrongMessage || "De data aceasta te iert. Te las să treci mai departe.")}"></label></div>`;
